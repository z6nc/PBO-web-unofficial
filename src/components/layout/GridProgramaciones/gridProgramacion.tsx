                    ...
                    filteredColaboradores.slice(0, numeroGRID).map((colaborador, index) => (
                        <div key={index} className={`relative ${colaborador.estilo} rounded-lg overflow-hidden group `}>
                            ...
                            <div className="absolute flex flex-col  z-20 text-white bottom-0 w-60 ">
                                <span className="bg-PBO/60 py-2 text-sm md:text-base  group-hover:bg-PBO/80 px-4 mr-auto">{colaborador.horarios ?? colaborador.rol}</span>
                                <h4 className=" bg-BlackPBO p-2 font-semibold text-base md:text-lg font-Monserat ">{colaborador.programa ?? colaborador.NombreCompleto}</h4>
                            </div>
                        </div>
                    ))
                }
